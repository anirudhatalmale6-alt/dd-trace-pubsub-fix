...
import static osmos.commerce.marketplace.orderorchestrator.common.utils.HeaderUtils.DD_TRACE_ID;
...
public class PubSubPublishTraceAspect {
    ...
    public void injectTraceId(JoinPoint joinPoint) {
        ...
        if (args.length >= 3 && args[2] instanceof Map) {
            ...
            if (traceId != null && !traceId.equals("0") && !attributes.containsKey(DD_TRACE_ID)) {
                attributes.put(DD_TRACE_ID, traceId);
            }

            // Also inject x-datadog-* headers for downstream artifact trace propagation
            ...
            log.info("Injecting dd.trace_id {} and x-datadog-trace-id into PubSub attributes", traceId);
