package osmos.commerce.shipping.aspect;
    ...
    public void injectTraceId(JoinPoint joinPoint) {
        ...
        if (args.length >= 3 && args[2] instanceof Map) {
            ...
            if (traceId != null && !traceId.equals("0")) {
                if (!attributes.containsKey("dd.trace_id")) {
                    attributes.put("dd.trace_id", traceId);
                }
                if (!attributes.containsKey("x-datadog-trace-id")) {
            ...
            log.info("PubSub publish intercepted - dd.trace_id={}, x-datadog-trace-id={}", traceId, traceId);
        }
    }
